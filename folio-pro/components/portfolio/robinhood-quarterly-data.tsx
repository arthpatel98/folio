...
export function RobinhoodQuarterlyData({ onAllTimeSummary }: { onAllTimeSummary?: (summary: RobinhoodAllTimeSummary) => void }) {
  ...
  const BarValueLabel=(props:any)=>{const {x,y,width,height,value}=props;if(value===undefined||value===null)return null;const numericValue=Number(value);const isNegative=numericValue<0;const barY=Number(y);const barHeight=Number(height);const barBottom=Math.max(barY,barY+barHeight);const labelY=isNegative?barBottom+10:Math.min(barY,barY+barHeight)-8;return <text x={Number(x)+Number(width)/2} y={labelY} textAnchor="middle" dominantBaseline={isNegative?"hanging":"auto"} fill={isNegative?"#fb7185":"#e4e4e7"} fontSize={11} fontWeight={700} stroke="#09090b" strokeWidth={2} paintOrder="stroke">{wholeDollar(numericValue)}</text>;};
  return <>
    <Card className="overflow-hidden">
      ...
      <CardContent className="pt-5"><div className="h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={data} barGap={6} barCategoryGap="22%" margin={{left:4,right:12,top:28,bottom:34}} onClick={(state:any)=>{const period=state?.activePayload?.[0]?.payload?.period;if(typeof period==="string")open(period);}} style={{cursor:"pointer"}}>
