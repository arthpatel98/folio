...
export function RobinhoodQuarterlyData({ onAllTimeSummary }: { onAllTimeSummary?: (summary: RobinhoodAllTimeSummary) => void }) {
  ...
  const transactionsByPeriod=useMemo(()=>{
    ...
    activeTransactions.forEach((tx:Transaction)=>{
      ...
      const category:ProfitDrilldownTransaction["category"]=option
        ...
        : "Common Stocks";
      const holding=holdings.find(item=>item.symbol.toUpperCase()===tx.symbol!.toUpperCase()&&Boolean(item.assetType==="option")===option);
      ...
      const optionLabel=option
        ? [tx.symbol.toUpperCase(),tx.optionStrike!==undefined?`$${tx.optionStrike}`:"",optionKind.includes("put")?"Put":"Call",expiryText?`Exp ${expiryText}`:""].filter(Boolean).join(" ")
        : (holding?.company||tx.symbol.toUpperCase());
      const liveTx:ProfitDrilldownTransaction={id:`live-${tx.id}`,date:tx.date,ticker:tx.symbol.toUpperCase(),label:option?(tx.optionSymbol||optionLabel):optionLabel,quantity:tx.quantity??null,price:tx.price??null,proceeds:tx.realizedProceeds??null,realizedProfit:tx.realizedGain,category,preserveLabelCasing:true};
  ...
  const BarValueLabel=(props:any)=>{const {x,y,width,value}=props;if(value===undefined||value===null)return null;return <text x={Number(x)+Number(width)/2} y={Number(y)-7} textAnchor="middle" fill="#e4e4e7" fontSize={11} fontWeight={700} stroke="#09090b" strokeWidth={2} paintOrder="stroke">{wholeDollar(Number(value))}</text>;};
  return <>
