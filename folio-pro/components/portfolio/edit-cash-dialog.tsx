...
export function EditCashDialog() {
  ...
  useEffect(() => { if (!open) setValue(String(cash)); }, [cash, open]);
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    ...
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/10 bg-zinc-950 p-6 shadow-2xl">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4"><label className="space-y-2 text-sm font-medium text-zinc-300"><span>Cash amount</span><Input type="number" min="0" step="any" value={value} onChange={(e) => setValue(e.target.value)} autoFocus /></label>{error && <p className="rounded-lg border border-rose-500/20 bg-rose-500/10 px-3 py-2 text-sm text-rose-300">{error}</p>}<div className="flex justify-end gap-3"><Dialog.Close asChild><Button type="button" variant="outline">Cancel</Button></Dialog.Close><Button type="submit">Save cash</Button></div></form>
