...
export function EditCashDialog({ children }: { children?: ReactNode }) {
  ...
  return <Dialog.Root open={open} onOpenChange={setOpen}>
    <Dialog.Trigger asChild>{children ? <button type="button" className="rounded-lg text-right transition hover:bg-white/[.04] focus:outline-none focus:ring-2 focus:ring-emerald-400/30" aria-label="Edit Cash" title="Click To Edit Cash">{children}</button> : <button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200 text-zinc-500 hover:bg-zinc-100 dark:border-white/10 dark:hover:bg-white/10" aria-label="Edit cash"><Pencil size={14} /></button>}</Dialog.Trigger>
    <Dialog.Portal><Dialog.Overlay className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm" /><Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/10 bg-zinc-950 p-4 sm:p-6 shadow-2xl">
      <div className="flex items-start justify-between"><div><Dialog.Title className="text-xl font-semibold text-white">Edit Cash Balance</Dialog.Title></div><Dialog.Close className="rounded-lg p-2 text-zinc-400 hover:bg-white/5 hover:text-white"><X size={18} /></Dialog.Close></div>
