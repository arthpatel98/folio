...
function SectorField({ value, onChange }: { value: "" | Sector; onChange: (value: string) => void }) {
  return <Field label="Sector"><select required value={value} onChange={(e) => onChange(e.target.value)} className="field-select"><option value="">Select Sector</option>{sectors.map((sector) => <option key={sector} value={sector}>{sector}</option>)}</select></Field>;
}
