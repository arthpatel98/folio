    ...
    <Card className="overflow-hidden">
      <div className="space-y-4 border-b border-white/10 p-4 sm:p-5">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_190px_165px_165px]">
          ...
          <select value={typeFilter} onChange={e=>setTypeFilter(e.target.value as "all"|TransactionType)} className="h-10 rounded-xl border border-white/10 bg-zinc-950 px-3 text-sm outline-none"><option value="all">All transaction types</option>{Object.entries(TYPE_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>
