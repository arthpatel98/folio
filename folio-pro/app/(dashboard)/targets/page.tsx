...
export default function TargetPlannerPage(){
  ...
  const details=owned.map(h=>{
    ...
    const existingProfit=(s.targetPrice-h.averageCost)*ownedQty*multiplier*direction;
    ...
    const expectedReturn=h.averageCost>0?((s.targetPrice-h.averageCost)/h.averageCost)*100*direction:0;
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="font-semibold">1. Select Target Date</h2><p className="mt-1 text-sm text-zinc-500">Choose One Of Your Saved Target Portfolio Values Milestones.</p></div><select value={selectedDate} onChange={e=>setSelectedDate(e.target.value)} className="h-11 rounded-xl border border-white/10 bg-zinc-950 px-4 text-sm outline-none">{rows.map(r=><option key={r.date} value={r.date}>{r.date}</option>)}</select></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><Mini label="Starting Value" value={money2(currentValue)}/><Mini label="Profit Needed" value={money(baseGap)}/><Mini label="Selected Portfolio" value={active.name}/></div></Card>
